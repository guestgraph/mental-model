...
**Only what is built from this model is a surface.** The MCP server, its Registry listing and the chat are, built by `guestgraph/mcp-guestgraph-io` from a pinned commit of this repository. guestgraph.io is not: its pages are written from the services and the talks. A place that is planned and not yet reachable has no file until it is.
